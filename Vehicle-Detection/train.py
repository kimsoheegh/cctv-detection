...
# import wandb
# wandb.init(project="kcar_2", entity="sohee726")
#
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if isinstance(hyp, str):
        with open(hyp, "rt", encoding="UTF-8") as f:
    ...
    is_coco = data.endswith('coco.yaml') and nc == 79  # COCO dataset
    ...
    hyp['cls'] *= nc / 79. * 3. / nl  # scale to classes and layers
...
def parse_opt(known=False):
    ...
    parser.add_argument('--quad', action='store_true', help='quad woader')
...
def main(opt):
    ...
    if opt.resume and not check_wandb_resume(opt) and not opt.evolve:  # resume an interrupted run
        ...
        with open(Path(ckpt).parent.parent / 'opt.yaml', "rt", encoding="UTF-8") as f:
    ...
    # Evolve hyperparameters (optional)
        ...
        with open(opt.hyp, "rt", encoding="UTF-8") as f:
            hyp = yaml.safe_load(f)  # load hyps dict
        ...
        if opt.bucket:
            os.system(f'gsutil cp gs://{opt.bucket}/evolve.csv {save_dir}')  # download evolve.csv if existsrun
